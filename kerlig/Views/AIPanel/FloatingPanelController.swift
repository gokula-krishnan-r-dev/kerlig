...
class FloatingPanelController: NSObject, NSWindowDelegate {
  ...
  private func restartAccessibilityService(completion: @escaping (Bool) -> Void) {
    DispatchQueue.global(qos: .userInitiated).async {
      ...
      let hasPermission =
        self.hotkeyManager.hasAccessibilityPermission()
  ...
  func showUnifiedPanelWithContext(
    appState: AppState
  ) {
    if panel == nil {
      ...
      // Set up the content view with SwiftUI and specify initial tab
      ...
      if !NSPointInRect(screenLocation, panelFrame) && !self.isAttemptingTextCapture && !isPinned {
        DispatchQueue.main.async {
          ...
          NotificationCenter.default.post(
            name: NSNotification.Name("ClosePanelNotification"), object: nil)
        }
      }
    }
  }
  ...
  // Add a method to update the panel with an AI response
  func updateResponse(_ response: String) {
    // Access the panel's content view controller if it exists
    if let kerligPanel = panel,
      let contentView = kerligPanel.contentViewController
        as? NSHostingController<KerligStylePanelView>,
      let panelView = contentView.rootView as? KerligStylePanelView
    {

      // Update the panel view with the new response
      DispatchQueue.main.async {
        panelView.updateResponse(response)
      }
    }
  }

  // Add method to show panel with file details
  func showFileDetailsPanel(url: URL, appState: AppState) {
    // Store reference to the app state
    self.appStateRef = appState
