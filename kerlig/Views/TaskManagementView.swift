...
struct TaskManagementView: View {
    ...
    var body: some View {
        ZStack {
            ...
            if !authManager.isAuthenticated {
                ...
                // Show task management interface when authenticated
                authenticatedTaskView
        ...
        .onAppear {
            ...
            // Setup initial state only if authenticated
            if authManager.isAuthenticated {
                setupInitialState()
            }
        ...
        .onChange(of: authManager.isAuthenticated) { _, isAuthenticated in
            if isAuthenticated {
                // User just authenticated, setup initial state
                setupInitialState()
            } else {
    ...
    // MARK: - Authenticated Task View

    private var authenticatedTaskView: some View {
        ...
        HStack {
            ...
            VStack(alignment: .leading, spacing: 4) {
                if let user = authManager.currentUser {
                    Text("Welcome in, \(user.firstName)!")
                        .font(.system(size: 24, weight: .semibold))
                        .foregroundColor(.white)

                    Text("Manage Your Daily Tasks Effortlessly")
                        .font(.system(size: 14))
    ...
    private var userAvatarMenu: some View {
        Menu {
            if let user = authManager.currentUser {
                Section {
                    Label(user.fullName, systemImage: "person.circle")
...
//                }
                ...
                VStack {
                    if let imageUrl = user.profileImageUrl, !imageUrl.isEmpty {
                        AsyncImage(url: URL(string: imageUrl)) { image in
                            ...
                            userInitialsView(user.initials)
                        }
    ...
    private func userInitialsView(_ initials: String) -> some View {
        Text(initials)
                        ...
                        .background(
                            LinearGradient(
                                gradient: Gradient(colors: [
                                    successColor, successColor.opacity(0.8),
                                ]),
                                startPoint: .topLeading,
                                endPoint: .bottomTrailing
                            )
    ...
    private func createDefaultColumnsForRelease(_ release: Release) {
        let defaultColumnTitles = [
            "Backlog", "In Progress", "Today", "Review", "Done", "Cancelled",
        ]
    ...
    private func addQuickTask() {
        guard let release = selectedRelease,
            let firstColumn = taskColumns.first
        else { return }
        ...
        if let window = NSApp.keyWindow {
            alert.beginSheetModal(for: window) { response in
                if response == .alertFirstButtonReturn {
                    let columnTitle = inputTextField.stringValue.trimmingCharacters(
        ...
        VStack(spacing: 0) {
            ...
            VStack(spacing: 24) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    TextField(
                        "Enter release description", text: $newReleaseDescription, axis: .vertical
                    )
                    .textFieldStyle(PlainTextFieldStyle())
                    .font(.system(size: 16))
                    .foregroundColor(.white)
    ...
    private func createRelease() {
        guard let selectedProject = selectedProject,
            !newReleaseVersion.isEmpty && !newReleaseName.isEmpty
        else { return }
            ...
            ForEach(TabSection.allCases, id: \.self) { tab in
                Button(action: {
                    ...
                    VStack(spacing: 8) {
                        Text(tab.rawValue)
                            .font(
                                .system(size: 16, weight: selectedTab == tab ? .semibold : .regular)
                            )
