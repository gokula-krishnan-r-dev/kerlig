...
private struct ProjectReleasePreferences {
    ...
    static func loadSelectedProject(from projects: [Project], releases: [Release]) -> (
            ...
            if let savedProject = projects.first(where: { $0.id == projectId && !$0.isArchived }) {
                    ...
                    let releaseId = UUID(uuidString: releaseIdString)
                {
                    ...
                    if let savedRelease = projectReleases.first(where: { $0.id == releaseId }) {
                        print(
                            "🎯 Restored saved selection: \(savedProject.title) -> v\(savedRelease.version)"
                        )
                ...
                print(
                    "🔄 Project found, selecting first release: \(savedProject.title) -> \(firstRelease?.version ?? "None")"
                )
    ...
    static func getCurrentStoredPreferences() -> (
        projectId: String?, releaseId: String?, lastUsedProjectId: String?
    ) {
        let userDefaults = UserDefaults.standard
        return (
...
struct FloatingSidebarView: View {
            ...
            if !authManager.isAuthenticated {
                    ...
                    .background(
                        LinearGradient(
                            gradient: Gradient(colors: [
                                Color(hex: "#2E2E2E"), Color(hex: "#1A1A1A"),
                            ]),
                            startPoint: .top,
                            endPoint: .bottom
                        )
                ...
                if isCompleted {
                    ...
                    if showNotionContent {
                        ...
                        if !isAddingNote
                            && !noteStore.getFilteredPendingNotes(
                                selectedProject: selectedProject, selectedRelease: selectedRelease
                            ).isEmpty
                        {
                            orderWithAIButton
                        }
                ...
                taskListView
                if !isCompleted {
                    if noteStore.getFilteredPendingNotes(
                        selectedProject: selectedProject, selectedRelease: selectedRelease
                    ).count == 0 {
                        Spacer()
                        emptyStateView
                    ...
                    onSnooze: { minutes in
                        ...
                        updatedTask.scheduledTime =
                            Calendar.current.date(
                                byAdding: .minute, value: minutes,
                                to: updatedTask.scheduledTime ?? Date()) ?? Date()
        ...
        Button(action: {
            ...
            ZStack {
                ...
                Circle()
                    .fill(
                        LinearGradient(
                            gradient: Gradient(colors: [
                                ...
                                isFullScreenButtonHovered
                                    ? Color(hex: "#45A049").opacity(0.3)
                                    : Color(hex: "#262628").opacity(0.6),
                            ]),
                    ...
                    .scaleEffect(
                        isFullScreenButtonPressed ? 0.85 : (isFullScreenButtonHovered ? 1.1 : 1.0)
                    )
                    .shadow(
                        color: isFullScreenButtonHovered
                            ? Color(hex: "#4CAF50").opacity(0.3) : Color.black.opacity(0.2),
                ...
                Image(systemName: "arrow.up.left.and.arrow.down.right")
                    ...
                    .scaleEffect(
                        isFullScreenButtonPressed ? 0.8 : (isFullScreenButtonHovered ? 1.2 : 1.0)
                    )
                ...
                HStack(spacing: 8) {
                    ...
                    if let release = selectedRelease {
                        ...
                        Image(systemName: "shippingbox.fill")
                            .font(.system(size: 16))
                            .foregroundColor(
                                selectedProject != nil ? .orange.opacity(0.8) : .gray.opacity(0.5))
                    }
    ...
    private func taskTabBackground(for tab: TaskTab) -> some View {
        RoundedRectangle(cornerRadius: 8)
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 8)
                    .stroke(
                        selectedTaskTab == tab ? Color(hex: "#4CAF50").opacity(0.3) : Color.clear,
                        lineWidth: 0.8
                    )
            )
    }
                ...
                HStack(spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Time")
                            ...
                            .foregroundColor(.gray)
                        DatePicker(
                            "", selection: $scheduledTime, displayedComponents: .hourAndMinute
                        )
                        .datePickerStyle(.compact)
                        .colorScheme(.dark)
                    }
    ...
    private var taskProgressSummary: some View {
        let completedTasks = noteStore.getFilteredCompletedNotes(
            selectedProject: selectedProject, selectedRelease: selectedRelease)
        let pendingTasks = noteStore.getFilteredPendingNotes(
            selectedProject: selectedProject, selectedRelease: selectedRelease)
    ...
    private var aiEnhancementStatusView: some View {
        HStack(spacing: 8) {
            ...
            Text(
                aiEnhancementStatus.isEmpty
                    ? "Generating AI-enhanced content..." : aiEnhancementStatus
            )
            .font(.system(size: 12, weight: .medium))
    ...
    private var orderWithAIButton: some View {
        Button(action: {
            ...
            .background(
                RoundedRectangle(cornerRadius: 8)
                    .fill(
                        isAIReordering ? Color(hex: "#9333EA").opacity(0.1) : Color(hex: "#2C2C2E")
                    )
                    .overlay(
                        RoundedRectangle(cornerRadius: 8)
            ...
            LazyVStack(spacing: 4) {
                ...
                ForEach(
                    ...
                    .animation(
                        ...
                        value: recentlySkippedNoteId == note.id
                    )
                    .overlay(taskRowOverlay(for: note), alignment: .bottomTrailing)
                }

                // Scheduled tasks section
                let scheduledTasks = noteStore.getFilteredScheduledNotes(
                    selectedProject: selectedProject, selectedRelease: selectedRelease)
            ...
            .fill(
                note.id == recentlyMadeFirstNoteId
                    ...
                    : (note.id == recentlySkippedNoteId
                        ...
                        : (note.isScheduled
                            ? Color(hex: "#4CAF50").opacity(0.1) : Color(hex: "#2C2C2E")))
            )
            .overlay(
                RoundedRectangle(cornerRadius: 10)
                    .stroke(
    ...
    private var completedTasksSection: some View {
        let completedTasks = noteStore.getFilteredCompletedNotes(
            selectedProject: selectedProject, selectedRelease: selectedRelease)
        let totalTimeSpent = noteStore.getTotalTimeSpentOnFilteredCompletedNotes(
            selectedProject: selectedProject, selectedRelease: selectedRelease)
    ...
    private func expandedStatsHeader(tasks: [Note]) -> some View {
        HStack(spacing: 4) {
            ...
            VStack(alignment: .trailing, spacing: 2) {
                ...
                Text(
                    noteStore.getTotalTimeSpentOnFilteredCompletedNotes(
                        selectedProject: selectedProject, selectedRelease: selectedRelease)
                )
                .font(.system(size: 6, weight: .bold))
                .foregroundColor(.blue.opacity(0.9))
    ...
    private var focusModeButton: some View {
        Button(action: {
            ...
            .background(
                RoundedRectangle(cornerRadius: 20)
                    .fill(
                        LinearGradient(
                            gradient: Gradient(
                                colors: isFocusModeButtonHovered
                                    ? [Color(hex: "#3A3A3C"), Color(hex: "#2E2E30")]
                                    : [Color(hex: "#2C2C2E"), Color(hex: "#262628")]),
                            startPoint: .top,
                            endPoint: .bottom
                        )
                    )
            ...
            .shadow(
                color: isFocusModeButtonHovered
                    ? Color.green.opacity(0.35) : Color.black.opacity(0.15),
    ...
    private func initializeProjectSelection() {
        ...
        print(
            "✅ Project selection initialized: \(savedProject?.title ?? "None") -> \(savedRelease?.version ?? "None")"
        )
    }
    ...
    private func validateAndRefreshSelection() {
        ...
        if let currentRelease = selectedRelease {
            ...
            if !projectReleases.contains(where: { $0.id == currentRelease.id }) {
                ...
                selectedRelease = projectReleases.first
                ProjectReleasePreferences.saveSelectedProject(
                    selectedProject, release: selectedRelease)
                updateTaskData()
            }
        }
    }
    ...
    private func findFirstNote() -> Note? {
        return noteStore.getFirstPendingNote(
            selectedProject: selectedProject, selectedRelease: selectedRelease)
    }
        ...
        VStack(spacing: 0) {
            ZStack {
                ...
                VStack(spacing: 6) {
                    ...
                    GifImageView(
                        gifURL: getRandomCelebrationGif(
                            category: getCelebrationCategory(for: completedTaskTime))
                    )
                    ...
                    VStack(spacing: 12) {
                        Button(action: {
                            ...
                            if let nextNote = noteStore.getFilteredPendingNotes(
                                selectedProject: selectedProject, selectedRelease: selectedRelease
                            ).first {
    ...
    private func createNewNote() {
        if !newNoteTitle.isEmpty {
            ...
            let mediaToSave =
                showMediaPicker && taskMediaContent.hasContent
                ? taskMediaContent : MediaContent(emoji: getRandomEmoji())
            print("📝 [TASK-CREATE] ==========================================")
        ...
        self.aiService?.generateTaskDescriptionAndSubtasks(taskTitle: taskTitle) { result in
            ...
            DispatchQueue.main.async {
                ...
                case .success(let taskData):
                    ...
                    if !taskData.subtasks.isEmpty {
                        ...
                        for (index, subtask) in taskData.subtasks.enumerated() {
                            print(
                                "   \(index + 1). '\(subtask.title)' - \(subtask.priority) priority - \(Int(subtask.estimatedDuration/60))min"
                            )
                        }
                    }
                    ...
                    self.updateTaskWithAIContent(
                        taskId: taskId, taskData: taskData, originalTitle: taskTitle)
                ...
                case .failure(let error):
                    ...
                    self.createFallbackAIContent(
                        taskId: taskId, originalTitle: taskTitle, error: error)
                }
            }
        }
    ...
    private func createFallbackAIContent(taskId: UUID, originalTitle: String, error: Error) {
        ...
        note.aiGeneratedDescription =
            "AI generation failed: \(error.localizedDescription). This is a debug placeholder description for task: \(originalTitle)"
    ...
    private func updateTaskWithAIContent(
        taskId: UUID, taskData: TaskEnhancementData, originalTitle: String
    ) {
        print("🤖 [AI-ENHANCE] STEP 6: Updating task with AI content...")
        print("🤖 [AI-ENHANCE] Task ID: \(taskId)")
        ...
        print(
            "   - Description: '\(originalDescription?.prefix(30) ?? "None")...' → '\(taskData.description.prefix(30))...'"
        )
        ...
        if let updatedNote = noteStore.notes.first(where: { $0.id == taskId }) {
            ...
            print(
                "   - Description Length: \(updatedNote.aiGeneratedDescription?.count ?? 0) chars")
    ...
    private func createScheduledTask() {
        if !newNoteTitle.isEmpty {
            ...
            let taskTitle = newNoteTitle  // Store title before reset
            let mediaToSave =
                (showMediaPicker && taskMediaContent.hasContent) ? taskMediaContent : nil
            ...
            print(
                "📅 [SCHEDULED-TASK] Media Content: \(mediaToSave?.hasContent == true ? "Yes" : "No")"
            )
    ...
    private func reorderTasksWithAI() {
        ...
        let tasksToReorder = noteStore.getReorderableTasks(
            selectedProject: selectedProject, selectedRelease: selectedRelease)
            ...
            DispatchQueue.main.async {
                ...
                case .success(let reorderingData):
                    ...
                    let reorderedTasks = reorderingData.reorderedTasks.map { task in
                        (
                            taskId: task.taskId, newPosition: task.newPosition,
                            priority: task.priority, reasoning: task.reasoning
                        )
                    }
    ...
    private func applyFallbackOrdering() {
        ...
        let reorderedTasks = sortedTasks.enumerated().map { index, task in
            (
                taskId: task.id.uuidString, newPosition: index + 1,
                priority: task.priority.rawValue.lowercased(),
                reasoning: "Fallback ordering by priority and creation date"
            )
        }
        ...
        noteStore.reorderNotesWithAI(
            reorderedTasks: reorderedTasks,
            reorderingSummary:
                "Tasks reordered using fallback priority-based sorting. Higher priority tasks are shown first.",
            selectedProject: selectedProject,
            selectedRelease: selectedRelease
        )
    ...
    private var modernRowBackground: some View {
        RoundedRectangle(cornerRadius: 8)
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 8)
                    .stroke(
                        isHovered ? Color.green.opacity(0.2) : Color.white.opacity(0.03),
                        lineWidth: 0.5
                    )
            )
    }
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            if noteStore.activeTimerNote == nil {
                ...
                print(
                    "ℹ️ [FocusCard] Timer already active for: \(noteStore.activeTimerNote?.title ?? "Unknown")"
                )
            }
            ...
            HStack(spacing: 8) {
                ...
                TaskActionButton(
                    ...
                    action: {
                        ...
                        print(
                            "✅ [Sidebar] Task moved to first position and activated: \(note.title)")
                    },
                    ...
                    if !isBreak {
                        ...
                        TaskActionButton(
                            ...
                            action: {
                                ...
                                print(
                                    "✅ [Sidebar] Task completed: \(note.title) in \(formatTime(elapsedTime))"
                                )
                            },
                            onHover: { isHovering in
        ...
        HStack(spacing: 12) {
            ...
            if note.mediaContent.hasContent {
                VStack {
                    switch note.mediaContent.type {
                    case .image:
                        if let imageData = note.mediaContent.imageData,
                    ...
                    LinearGradient(
                        gradient: Gradient(colors: [
                            firstNote?.id == note.id
                                ? Color(hex: "#2A332C") : Color(hex: "#2C2C2E"),
                            firstNote?.id == note.id
                                ? Color(hex: "#263026") : Color(hex: "#262628"),
                        ]),
                        startPoint: .top,
                        endPoint: .bottom
                    )
                ...
                RoundedRectangle(cornerRadius: 8)
                    .fill(
                        isHovered
                            ? LinearGradient(
                                gradient: Gradient(colors: [
                                    Color(hex: "#3C3C3E"), Color(hex: "#343436"),
                                ]),
                                startPoint: .top,
                                endPoint: .bottom
                            )
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    HStack(spacing: 8) {
                        ...
                        {
                            Label(
                                formatScheduledDateTime(date: scheduledDate, time: scheduledTime),
            ...
            RoundedRectangle(cornerRadius: 12)
                .fill(
                    LinearGradient(
                        gradient: Gradient(colors: [
                            firstNote?.id == note.id
                                ? Color(hex: "#2A4A2D") : Color(hex: "#2C2C2E"),
                            firstNote?.id == note.id
                                ? Color(hex: "#263026") : Color(hex: "#262628"),
                        ]),
                        startPoint: .top,
