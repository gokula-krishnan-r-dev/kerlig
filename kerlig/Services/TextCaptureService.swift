...
class TextCaptureService: ObservableObject {
  ...
  @Published var capturedFileDetails: [String: Any]? = nil
  ...
  private func setupNotifications() {
    // Listen for file detail notifications from HotkeyManager
    NotificationCenter.default.addObserver(
      forName: Notification.Name("FileDetailsSelected"),
      object: nil,
      queue: .main
    ) { [weak self] notification in
      if let fileDetails = notification.userInfo?["fileDetails"] as? [String: Any] {
        self?.capturedFileDetails = fileDetails

        // Also notify that file details have been captured
        NotificationCenter.default.post(
          name: Notification.Name("FileDetailsCaptured"),
          object: nil,
          userInfo: ["fileDetails": fileDetails]
        )
      }
    }
  }
  ...
  func captureSelectedText() {
    // Check if we're in Finder first, as file selection requires different handling
    if let frontmostApp = NSWorkspace.shared.frontmostApplication,
      frontmostApp.bundleIdentifier == "com.apple.finder"
    {
      // File selection will be handled by HotkeyManager
      return
    }
  ...
  private func captureTextUsingAccessibility(from app: NSRunningApplication) -> Bool {
    ...
    if selectedTextResult == AXError.success,
      ...
      let text = CFGetTypeID(cfString) == CFStringGetTypeID()
        ? (cfString as! CFString) as String : nil,
      !text.isEmpty
    {
